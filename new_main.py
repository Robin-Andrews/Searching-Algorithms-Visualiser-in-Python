        ...
        if program_state == "do_bfs":
            if bfs_queue:
                ...
                for i2, j2 in ((i + 1, j), (i - 1, j), (i, j + 1), (i, j - 1)): # Down, up, right, left
                    if 0 <= i2 < num_rows and 0 <= j2 < num_cols and maze[i2][j2] != "#" and (i2, j2) not in visited_positions:
...
def reset():
    ...
    global graphical_maze, num_rows, num_cols
...
def main():
    """
    The main function to control the program.
    Defines values needed for the program.
    Creates a logical and a graphical board
    Initializes gui and timer
    """
    ...
    # Determine path and length of path
    # path = search.bfs(maze, (0, 0), graphical_maze)
    # print(path)


    # Set GUI handlers and initilize GUI and timer
    config.frame.set_draw_handler(draw)
    # config.frame.set_mouseclick_handler(click)
    start_button = config.frame.add_button("Start", start, 25)
    reset_button = config.frame.add_button("Reset", reset, 25)
